    ...
    ($network_name:ident, $relay_chain:ty) => {
        ...
        impl $relay_chain {
            ...
            pub fn account_data_of(
                account: AccountIdOf<<Self as RelayChain>::Runtime>,
            ) -> $crate::AccountData<Balance> {
                Self::ext_wrapper(|| <Self as RelayChain>::System::account(account).data)
            }

            pub fn sovereign_account_id_of(
                location: $crate::MultiLocation,
            ) -> AccountIdOf<<Self as RelayChain>::Runtime> {
...
macro_rules! __impl_parachain {
    ($network_name:ident, $parachain:ty) => {
        ...
        impl $parachain {
            ...
            pub fn account_data_of(
                account: AccountIdOf<<Self as Parachain>::Runtime>,
            ) -> $crate::AccountData<Balance> {
            ...
            pub fn sovereign_account_id_of(
                location: $crate::MultiLocation,
            ) -> AccountIdOf<<Self as Parachain>::Runtime> {
                <Self as Parachain>::LocationToAccountId::convert(location.into()).unwrap()
            }

            pub fn fund_accounts(
                accounts: Vec<(AccountIdOf<<Self as Parachain>::Runtime>, Balance)>,
            ) {
                Self::ext_wrapper(|| {
                    for account in accounts {
...
pub mod helpers {
    ...
    pub fn weight_within_threshold(
        ...
        let ref_time_within = within_threshold(
            threshold_time,
            expected_weight.ref_time(),
            weight.ref_time(),
        );
        let proof_size_within = within_threshold(
            threshold_size,
            expected_weight.proof_size(),
            weight.proof_size(),
        );
